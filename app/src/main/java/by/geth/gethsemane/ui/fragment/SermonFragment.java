...
public class SermonFragment extends AudioFragment implements View.OnClickListener {

    public static SermonFragment newInstance(long id) {
        SermonFragment fragment = new SermonFragment();
        Bundle args = new Bundle();
        args.putInt(ARGS_UI_FLAGS, UI_FLAG_DISPLAY_HOME_AS_UP);
        args.putLong(ARGS_SERMON_ID, id);
        fragment.setArguments(args);
        return fragment;
    }

    public static final String ARGS_SERMON_ID = "ARGS_SERMON_ID";

    public static final String DATE_FORMAT_PATTERN = "dd.MM.yyyy";

    private long mSermonId;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mSermonId = args.getLong(ARGS_SERMON_ID);
        mSermon = new Select()
                .from(Sermon.class)
                .where(Sermon.COLUMN_ID + " = ?", mSermonId)
                .executeSingle();
        mAuthor = new Select()
				.from(AuthorEntity.class)
				.where(AuthorEntity.COLUMN_ID + " = ?", mSermon.getAuthorId())
                .executeSingle();
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        titleView.setText(mSermon.getTitle());

        String dateStr = dateFormat.format(mSermon.getDate());
        if (mAuthor == null) {
    ...
    public void onStart() {
        ...
        ApiService.getSermon(getContext(), mSermonId);
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        inflater.inflate(R.menu.fragment_sermon, menu);
    }

    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.fragment_sermon_menu_font_size:
                if (getContext() != null) {
                    new AlertDialog.Builder(getContext())
                            .setTitle(R.string.fragment_sermon_menu_font_size)
                            .setSingleChoiceItems(R.array.fragment_sermon_menu_font_size_items,
    ...
    protected void updateDownloadUI() {
        if (TextUtils.isEmpty(mSermon.getAudioLocalUri())) {
            if (DownloadController.getInstance().isDownloadInProgress(mSermon)) {
    ...
    private void updateContent() {
        if (!TextUtils.isEmpty(mSermon.getContent()))
            mContentView.setText(Html.fromHtml(mSermon.getContent()));
    }
    ...
    private BroadcastReceiver mApiServiceReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ApiService.ACTION_REQUEST_SUCCESSFUL.equals(action)
                    && ApiService.REQUEST_GET_SERMON.equals(requestType)) {
                mSermon = new Select()
                        .from(Sermon.class)
                        .where(Sermon.COLUMN_ID + " = ?", mSermonId)
                        .executeSingle();
